...
RELEASE = ROOT / "releases/hermes-0.20-ha-one-command-r3"
...
def main() -> int:
    ...
    evidence=load("evidence/ha-feature-summary.json")
    assert release["base_expected_tree"] == commits["base_tree"] == evidence["base_expected_tree"]
    assert release["active_base_commit"] == commits["active_base_commit"] == evidence["active_base_commit"]
    assert release["active_head_commit"] == commits["head_commit"] == evidence["active_head_commit"]
    assert release["expected_final_tree"] == commits["head_tree"] == evidence["expected_final_tree"]
    assert release["patch_count"] == commits["commit_count"] == patches["patch_count"] == 4
    assert release["total_patch_count_from_upstream_base"] == 37
    chain=commits["commits"]
    assert chain[0]["parents"] == [commits["active_base_commit"]]
    ...
    assert tests["autopilot_command_passed"] == 14
    assert tests["mission_regressions_passed"] == 100
    assert tests["failed"] == 0
    caps=evidence["capabilities"]
    assert all(caps.values())
    acceptance=evidence["acceptance"]
    assert all(item["status"] == "succeeded" for item in acceptance.values())
    assert evidence["remote_side_effects"] is False
    readme=(ROOT/"README.md").read_text()
    assert "`ha` one-command release r3" in readme
    assert release["active_head_commit"][:9] in readme
    print("Hermes ha one-command release r3 verified")
    return 0
