...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        print(json.dumps({
            ...
            "missions_reconciled": res.missions_reconciled,
            "mission_actions": res.mission_actions,
    ...
    if res.missions_reconciled:
        print(f"Missions reconciled: {', '.join(res.missions_reconciled)}")
    if res.mission_actions:
        actions = ", ".join(
            f"{item.get('mission_id', '?')}:{item.get('action', '?')}"
            for item in res.mission_actions
        )
        print(f"Mission actions: {actions}")
    if res.skipped_locked:
        print("Dispatch skipped: another dispatcher holds the board lock")
    return 0
