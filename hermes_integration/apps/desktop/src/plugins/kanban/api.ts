...
function withBoard(path: string, params: Record<string, string> = {}): string {
  return withBoardSlug(path, $boardSlug.get(), params)
}
...
export const missionsKey = (slug: string) => ['kanban', 'missions', slug] as const
...
export const fetchMissions = (slug = $boardSlug.get()) =>
  call<{ missions: Mission[] }>(withBoardSlug('/missions', slug))

export const fetchMission = (id: string, slug = $boardSlug.get()) =>
  call<MissionReport>(withBoardSlug(`/missions/${encodeURIComponent(id)}`, slug))
...
function autoNudge(slug = $boardSlug.get()): void {
  if (nudgeTimer != null) {
...
function nudged<T>(write: Promise<T>, slug = $boardSlug.get()): Promise<T> {
...
export const createMission = (body: Record<string, unknown>, slug = $boardSlug.get()) =>
  call<{ mission_id: string }>(withBoardSlug('/missions', slug), { method: 'POST', body })
...
export const missionAction = (
  ...
  nudged(
    call<{ mission: Mission }>(withBoardSlug(`/missions/${id}/actions`, slug), {
      method: 'POST',
      body: { action, ...extra }
    }),
    slug
...
export const nudgeDispatcher = (slug = $boardSlug.get()) =>
  call<{
    mission_actions: Array<{ mission_id: string; action: string }>
    missions_reconciled: string[]
    skipped_locked: boolean
    spawned: unknown[]
  }>(withBoardSlug('/dispatch', slug), { method: 'POST', body: {} })
